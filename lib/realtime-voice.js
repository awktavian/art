 ...
 * =============================================================
 * Connects to the OpenAI Realtime proxy resolved by lib/realtime-endpoint.js
 ...
 *     proxyUrl: resolveRealtimeEndpoint('voice', { params: { project: 'robo-skip' } }),
...
class RealtimeVoice {
  constructor(opts = {}) {
    // No defaults for the two options that decide WHO you are talking to.
    // `ws://localhost:8766` as a fallback proxyUrl meant a published page
    // silently dialled a port that only exists on a developer's laptop, and
    // reported "Connection failed" rather than "no endpoint configured".
    // `alloy` as a fallback voice silently replaced the project's colony voice
    // (lib/kagami-voices.js) whenever that config failed to load, so a missing
    // persona was indistinguishable from Kagami's own.
    if (typeof opts.proxyUrl !== 'string' || !opts.proxyUrl) {
      throw new TypeError(
        'RealtimeVoice: opts.proxyUrl is required — resolve it with ' +
          "resolveRealtimeEndpoint('voice', ...) from lib/realtime-endpoint.js. " +
          'There is no localhost default: a published page cannot reach :8766.'
      );
    }
    if (typeof opts.voice !== 'string' || !opts.voice) {
      throw new TypeError(
        'RealtimeVoice: opts.voice is required — it comes from the project entry in ' +
          'lib/kagami-voices.js (PROJECT_VOICES → colony → voice). Refusing to ' +
          'substitute a default persona for a missing one.'
      );
    }
