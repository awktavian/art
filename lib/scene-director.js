 ...
 *     proxyUrl: resolveRealtimeEndpoint('director'),   // lib/realtime-endpoint.js
 *     onAction: (action) => { ... },
...
class SceneDirector {
  constructor(opts = {}) {
    // The scene director has no deployed proxy — realtime-proxy/fly.toml
    // publishes only the voice relay on 8766. A `ws://localhost:8767` default
    // therefore pointed a published page at a port that cannot exist there.
    // Resolve it with resolveRealtimeEndpoint('director'), which throws
    // RealtimeEndpointUnavailable naming the service instead.
    if (typeof opts.proxyUrl !== 'string' || !opts.proxyUrl) {
      throw new TypeError(
        'SceneDirector: opts.proxyUrl is required — resolve it with ' +
          "resolveRealtimeEndpoint('director') from lib/realtime-endpoint.js. " +
          'realtime-proxy/claude-proxy.js (port 8767) has no deployed endpoint, ' +
          'so there is no default that works off localhost.'
      );
    }
