 ...
 *
 * Usage:
 *   <script src="../lib/realtime-endpoint.js"></script>
...
const VoiceOverlay = (() => {
  ...
  async function connect() {
    if (typeof window.resolveRealtimeEndpoint !== 'function') {
      throw new Error(
        'VoiceOverlay: lib/realtime-endpoint.js is not loaded. Add ' +
          '<script src="../lib/realtime-endpoint.js"></script> before voice-overlay.js — ' +
          'the overlay will not guess a proxy URL.'
      );
    }
    if (typeof window.buildVoiceConfig !== 'function') {
      throw new Error(
        'VoiceOverlay: lib/kagami-voices.js is not loaded, so no colony persona can be ' +
          'resolved for project "' + opts.project + '". Add ' +
          '<script src="../lib/kagami-voices.js"></script> before voice-overlay.js.'
      );
    }

    const config = window.buildVoiceConfig(opts.project);
    // A generic "helpful voice assistant" default silently replaced the whole
    // colony persona whenever a project key was absent from PROJECT_VOICES, and
    // the page looked like it had connected correctly. Name the gap instead.
    if (!config || !config.voice || !config.instructions) {
      throw new Error(
        'VoiceOverlay: project "' + opts.project + '" has no voice persona in ' +
          'lib/kagami-voices.js (PROJECT_VOICES). Refusing to substitute a generic assistant.'
      );
    }
    const colony = config.colony;

    const proxyUrl = window.resolveRealtimeEndpoint('voice', {
      override: opts.proxyUrl,
      params: { project: opts.project, colony: colony?.colony?.toLowerCase() },
    });

    voice = new RealtimeVoice({
      proxyUrl,
      voice: config.voice,
      instructions: config.instructions + (opts.extraInstructions ? '\n' + opts.extraInstructions : ''),
      tools: opts.tools || [],
      onTranscript,
      ...
      elements.btn.addEventListener('click', async () => {
        if (connected) { disconnect(); }
          ...
          try { await connect(); } catch (err) {
            ...
            // The old text named :8766 unconditionally, which is wrong on every
            // published page. Report the endpoint that was actually attempted.
            const where = window.describeRealtimeEndpoint
              ? window.describeRealtimeEndpoint('voice', { override: opts.proxyUrl })
              : '(lib/realtime-endpoint.js not loaded)';
            addLine((err?.message || 'Connection failed') + ' — endpoint: ' + where, '#f87171');
          }
        }
      });
