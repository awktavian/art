...
//
// Features:
//   - Ed25519 identity management with Keychain persistence
...
//
// Usage:
//   let meshService = MeshService.shared
//   try await meshService.initialize()
//   let peerId = meshService.peerId
//   let signature = try meshService.sign(message: data)
//
// h(x) >= 0. Always.
//
...
public enum MeshServiceError: LocalizedError {
    case notInitialized
...
public enum MeshConnectionState: String, CaseIterable, Sendable {
    ...
    static func from(_ rustState: String) -> MeshConnectionState {
        switch rustState {
...
public enum VectorClockOrdering: String, Sendable {
    ...
    static func from(_ result: String) -> VectorClockOrdering {
        switch result {
...
/// - Ed25519 identity management with secure Keychain storage
...
public final class MeshService: ObservableObject {
    ...
    private enum KeychainKey: String {
        ...
        case x25519PublicKey = "meshX25519PublicKey"
    }
    ...
    private let keychainService = "com.kagami.mesh"
    ...
    /// The identity is persisted in the iOS Keychain for security.
    ///
    ...
    public func resetIdentity() throws {
        ...
        x25519KeyPair = nil
        deleteX25519KeyPairFromKeychain()
    ...
    ///
    /// The keypair is stored in Keychain for persistence across app restarts.
    /// Both the secret and public keys are stored to enable proper restoration.
    ///
    /// - Returns: The public key hex for sharing with peers
    /// - Throws: `MeshServiceError.identityStoreFailed` if keychain storage fails
    public func generateX25519KeyPair() throws -> String {
        let keyPair = generateX25519Keypair()
        x25519KeyPair = keyPair

        // Store both keys in Keychain for proper persistence
        try saveX25519KeyPairToKeychain(keyPair)
    ...
    private func loadIdentityFromKeychain() -> MeshIdentity? {
        guard let base64 = getFromKeychain(key: .meshIdentity) else {
            return nil
        }
    ...
    private func saveIdentityToKeychain(_ identity: MeshIdentity) throws {
        ...
        if !saveToKeychain(key: .meshIdentity, value: base64) {
            throw MeshServiceError.identityStoreFailed("Keychain write failed")
        }
    }

    private func loadX25519KeyPairFromKeychain() -> X25519KeyPair? {
        guard let secretKeyHex = getFromKeychain(key: .x25519PrivateKey),
              let publicKeyHex = getFromKeychain(key: .x25519PublicKey) else {
            return nil
        }

        // Both keys must be present and valid
        guard !secretKeyHex.isEmpty, !publicKeyHex.isEmpty else {
            // Clean up partial data
            deleteFromKeychain(key: .x25519PrivateKey)
            deleteFromKeychain(key: .x25519PublicKey)
            return nil
        }

        return X25519KeyPair(secretKeyHex: secretKeyHex, publicKeyHex: publicKeyHex)
    }

    private func saveX25519KeyPairToKeychain(_ keyPair: X25519KeyPair) throws {
        // Save both keys atomically - if either fails, clean up
        let secretSaved = saveToKeychain(key: .x25519PrivateKey, value: keyPair.secretKeyHex)
        let publicSaved = saveToKeychain(key: .x25519PublicKey, value: keyPair.publicKeyHex)

        if !secretSaved || !publicSaved {
            // Clean up on failure
            deleteFromKeychain(key: .x25519PrivateKey)
            deleteFromKeychain(key: .x25519PublicKey)
            throw MeshServiceError.identityStoreFailed("Failed to save X25519 keypair to Keychain")
        }
    }

    private func deleteX25519KeyPairFromKeychain() {
        deleteFromKeychain(key: .x25519PrivateKey)
        deleteFromKeychain(key: .x25519PublicKey)
    }
    ...
    @discardableResult
    private func deleteFromKeychain(key: KeychainKey) -> Bool {
        let query: [String: Any] = [
            kSecClass as String: kSecClassGenericPassword,
            kSecAttrService as String: keychainService,
            kSecAttrAccount as String: key.rawValue
