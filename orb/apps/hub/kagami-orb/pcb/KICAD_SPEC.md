...
────────┼───────────────┼─────────────────────
0x0B    | BQ40Z50       | Fuel gauge (SBS standard address)
0x35    | sensiBel      | MEMS mic configuration
0x44    | SHT45         | Temperature/Humidity (optional)
0x48    | TMP117        | Temperature
0x60    | MCP4725       | DAC (height control setpoint)
0x68    | ICM-45686     | IMU (AD0=LOW)
0x6A    | BQ25895       | Battery charger

Note: BQ40Z50 uses the SBS 1.1 standard address 0x0B.
      BQ25895 uses default address 0x6A (ADDR pin floating).
```
