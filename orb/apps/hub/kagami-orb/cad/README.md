# Kagami Orb V3.1 — CAD Files & Fabrication Guide
...
## DXF/SVG Export Workflow

### Exporting DXF from OpenSCAD

For 2D parts (laser cutting, CNC routing):

```bash
# Export 2D projection as DXF
openscad -o output.dxf input.scad

# For specific projections, add to .scad file:
# projection(cut = true) translate([0,0,-z_slice]) your_model();
```

### Exporting SVG from OpenSCAD

```bash
# Export 2D projection as SVG
openscad -o output.svg input.scad
```

### DXF Export Notes

1. **Units:** All DXF files use millimeters. Verify scale in CAM software.
2. **Layers:** Use OpenSCAD `color()` to create layers that map to DXF layers.
3. **Closed Paths:** Ensure all profiles are closed for proper CAM toolpaths.
4. **Tolerance:** DXF export uses $fn=100 for smooth curves. Increase if needed.

### SVG Export Notes

1. **Viewbox:** SVG viewbox is set to model bounds. Verify dimensions.
2. **Stroke Width:** Default stroke is 0.1mm. Adjust in vector editor if needed.
3. **Fill:** OpenSCAD exports filled shapes. Convert to strokes for cutting.
4. **Glowforge:** Import SVG directly. Set cut/engrave per color/layer.

### Fusion 360 Import Workflow

1. Open Fusion 360
2. File → Import → Select `.step` or `.dxf` file
3. Verify dimensions (should be in mm)
4. Create CAM setup with appropriate toolpaths
5. Post-process for your machine

### Recommended Export Settings

| File Type | $fn | $fa | $fs | Use Case |
|-----------|-----|-----|-----|----------|
| STL (print) | 100 | 1 | 0.5 | 3D printing |
| DXF (laser) | 200 | 0.5 | 0.2 | Laser cutting |
| SVG (laser) | 200 | 0.5 | 0.2 | Glowforge/LightBurn |
| STEP (CAD) | 100 | 1 | 0.5 | CAD exchange |
