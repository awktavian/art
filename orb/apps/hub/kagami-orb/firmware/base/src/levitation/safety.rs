...
//!
//! # Safety Constraints (Physical CBF)
//!
//! - h_height_upper: Height <= 25mm (prevent collision with ceiling)
//! - h_height_lower: Height >= 5mm (prevent collision with base)
//! - h_descent_rate: Descent <= 15 mm/s (controlled landing)
//! - h_oscillation: Oscillation <= 5mm peak-to-peak (stability)
//! - h_thermal: Coil temperature <= 80C (fire prevention)
//! - h_power: Power supply must be OK (graceful shutdown)
//!
//! When h(x) approaches 0, the system takes corrective action.
//! When h(x) < 0, emergency procedures are triggered.
//!
//! # Privacy CBF (h_privacy) - NOT IMPLEMENTED IN FIRMWARE
//!
//! The privacy barrier function h_privacy(x) >= 0 is enforced at the
//! application layer (kagami-hub, kagami-ios, etc.), NOT in the base
//! station firmware. This is intentional:
//!
//! 1. The base station has no sensors that capture personal data
//!    (Hall effect sensor measures magnetic field, not people)
//! 2. Privacy decisions require context the firmware lacks
//!    (user consent, household membership, time-of-day preferences)
//! 3. The firmware only controls physical parameters
//!    (height, temperature, power - all physics, not privacy)
//!
//! Privacy enforcement is the responsibility of:
//! - kagami-hub: SafetyVerifier with h_privacy constraint
//! - kagami-ios/android: Consent management before data sharing
//! - kagami_smarthome: Camera/sensor access control
//!
//! See: CLAUDE.md "The Shadow" section for the full privacy philosophy.
