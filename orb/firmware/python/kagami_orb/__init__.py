"""
Kagami Orb Hardware Drivers

Python drivers for the Kagami Orb V3 hardware components.

Target Platform: Qualcomm QCS6490 SoM (Linux)

Components:
- HD108 LED ring (16-bit RGB, SPI)
- BQ25895 charger + BQ40Z50 fuel gauge (I2C)
- ICM-45686 IMU + VL53L8CX ToF + SHT45 environment (I2C)
- Hailo-10H NPU (PCIe/USB)
- Cellular modem (LTE/5G, USB/UART)
- GNSS receiver (Multi-constellation)

Usage:
    from kagami_orb import OrbSystem
...
class OrbSystem:
    ...
    def get_state(self) -> OrbSystemState:
        ...
        return OrbSystemState(
            ...
            humidity_pct=sensor_state.environment.humidity_percent,
            motion_detected=abs(sensor_state.imu.accel_x) > 0.5 or abs(sensor_state.imu.accel_y) > 0.5,
            proximity_mm=sensor_state.tof.closest_mm,
