...
The proof is an elementary induction requiring only $u^2 = v^2 = -1$ and $uv + vu = 0$.
Among normed division algebras over $\R$, this yields a threshold:
Fermat's equation has nontrivial solutions in every dimension $\geq 4$,
and none in dimension $1$ (Wiles) or $2$ (conditionally, via
...
Throughout, a \emph{nontrivial} solution means $a, b, c$ are all nonzero
and at least one has nonzero imaginary part (i.e., the solution does not
lie in $\Z$).
...
\]
where $\varepsilon = +1$ when $p \equiv 1, 7 \pmod{8}$ and
$\varepsilon = -1$ when $p \equiv 3, 5 \pmod{8}$.
\end{theorem}
...
Among normed division algebras over $\R$ of dimension $\geq 4$, the equation
$a^p + b^p = c^p$ (odd $p \geq 3$) has nontrivial solutions in the integer ring.
In dimension $1$ ($\R$), there are no solutions (Wiles~\cite{Wiles1995}).
In dimension $2$ ($\C$), there are no solutions for $p \leq 4$, and none for
$p \geq 5$ assuming Serre's modularity conjecture over $\Q(i)$
(Turcas~\cite{Turcas2018}).
\end{theorem}

The forward direction is Theorem~\ref{thm:main}.
The reverse direction combines Wiles' theorem~\cite{Wiles1995}
($\R$, dimension~1) with
...
The full formalization spans 169 Lean~4 files and 75{,}792 lines,
with axiom-free proofs for the core results.
All former axioms have been eliminated---converted to theorems, some with
\texttt{sorry} bodies exposing the precise mathematical gaps that remain.
...
For $p = 3$, FLT over $\Z$ follows from Euler's descent
in $\Z[\omega]$ ($\omega = e^{2\pi i/3}$; see Ribenboim~\cite{Ribenboim1979},
Chapter~9), and our Lean formalization verifies computationally that no
nontrivial solution exists in $\Z[i]$ with coordinates in $\{-2, \ldots, 2\}$
(\texttt{DimensionThreshold.lean}).
For general odd~$p \geq 5$, FLT over $\Z[i]$ is not unconditionally proved.
Any solution with all-real entries is a solution in $\Z$,
contradicting Wiles.
For solutions with nonzero imaginary parts, one needs modularity
of the Frey curve over $\Q(i)$.
Since $\Q(i)$ is imaginary quadratic (not totally real),
the Freitas--Le~Hung--Siksek theorem~\cite{FLHS2015} does not apply.
Turcas~\cite{Turcas2018} proves FLT over $\Q(i)$ for all primes
$p \geq 5$ \emph{conditionally} on Serre's modularity conjecture
over imaginary quadratic fields.
The unconditional case remains open, pending full modularity
of elliptic curves over $\Q(i)$ (Bianchi modular forms).
Our Lean formalization verifies computationally that no
nontrivial solution exists in $\Z[i]$ with coordinates in $\{-2, \ldots, 2\}$
for small exponents (\texttt{DimensionThreshold.lean}).
\end{proof}
...
\begin{remark}
This construction reduces to a scalar identity: all fourth powers are
positive reals, so the equation $a^4 + b^4 = c^4$ becomes $\Norm{a}^2 + \Norm{b}^2 = \Norm{c}^2$.
The non-commutativity of $\HH$ plays no role; the construction works identically
in $\OO_\Z$, $\Se_\Z$, or any algebra where purely imaginary elements satisfy
$q^4 = \Norm{q}^2$.
...
\]
This is always a real scalar. The ratio $r_n = (\alpha^n + \beta^n)/(-2)^n$
satisfies the linear recurrence $r_n = 2r_{n-1} - 3r_{n-2}$
(since $\alpha, \beta$ have trace $-2$ and norm~$4$),
with characteristic roots $(1 \pm i\sqrt{2})$ of modulus $\sqrt{3}$
and argument $\pm \arctan(\sqrt{2})$.
The period-6 pattern follows from $e^{6i\arctan(\sqrt{2})} = 1$
(verified numerically and for $n \leq 20$ algebraically).
Cases (i)--(ii) produce Fermat triples; (iii) does not but is handled by
Theorem~\ref{thm:sextic}.
\end{proof}
...
\begin{tabular}{@{}cllll@{}}
  ...
  4 & $\OO$ (dim 8) & associativity & $G_2$ & Local: Gan--Savin~\cite{GanSavin2023}; global: open \\
  \bottomrule
\end{tabular}
\end{table}
...
\noindent
Steps 1--4 are open, and Step~1 is the most fundamental:
no construction of a Frey-type geometric object from quaternionic
Fermat data is known. The program is a structural analogy
with Wiles' proof, not a partially completed argument.
Step~5 is proved.
...
\noindent
Totals: 169 files, 75{,}792 lines across the full formalization
(build clean as of 2026-03-27).
...
\begin{enumerate}
  \item \textbf{Pure type-theoretic proofs} (no \texttt{native\_decide},
    no \texttt{sorry}) for the main theorems
    (Theorems~\ref{thm:power-reduction}, \ref{thm:allodd}, \ref{thm:zerodiv}).
    These use only ring axioms and structural induction, checked entirely
    by Lean's type-checking kernel.
    They are valid in any ring satisfying the hypotheses.
  \item \textbf{Computational verification} via \texttt{native\_decide} for concrete instances:
    specific triples in $\HH_\Z$ and $\OO_\Z$, all 21 family verifications,
    and the Gaussian integer exhaustive search.
    These compile decision procedures to native code (introducing a trust
    assumption on the Lean compiler) and verify that the abstract hypotheses
    are satisfied by the concrete algebras.
...
\texttt{sorry} markers on three deep results: Frey curve modularity (\texttt{frey\_octic\_modular}),
Jacquet--Langlands transfer (\texttt{jl\_special\_transfer}),
and level lowering (\texttt{level\_lowering\_shimura}).
...
The broader FLT formalization carries 10 additional \texttt{sorry} markers in
Galois representation infrastructure (Tate modules, division polynomials,
level lowering, and automorphic bridge).
...
across the entire formalization.
All 10 former axioms---encoding Faltings' theorem, Ribet--Edixhoven level lowering,
Jacquet--Langlands, and Galois representation properties---have been
converted to theorems with \texttt{sorry} bodies, exposing the precise
mathematical gaps for future attack.
...
\begin{remark}[$G_2$ Selmer averaging --- speculative]
By analogy with the Bhargava--Shankar theorem
($\mathrm{avg}\,|\Sel_2(E)| = 3$ for elliptic curves),
one might ask whether $G_2$ Selmer groups admit an averaging result.
$G_2$ Selmer groups have not been explicitly defined
in the literature; our \texttt{G2SelmerGroup.lean} is an exploratory first attempt.
No specific prediction is made.
\end{remark}
...
\begin{conjecture}[Sedenion Even-Exponent Fermat --- S3]\label{conj:sedenion}
For even $n \geq 8$ with $n \equiv 0 \pmod{4}$, the equation
$a^n + b^n = c^n$ has no nontrivial solutions in $\Se_\Z$.
\end{conjecture}

\noindent
\emph{Context.} Mechanism~A gives odd-exponent solutions (via anti-commuting pairs
in quaternionic subalgebras). Mechanism~B fails for sedenions
(Section~\ref{sec:sedenion}). The remaining open case is the even-exponent one.
...
\begin{remark}[$\varphi$-contraction and $G_2$ geometry --- speculative]
...
is formally analogous to identities satisfied by the associative 3-form
on $G_2$-holonomy manifolds (Bryant~\cite{Bryant1987}, Joyce~\cite{Joyce2000}).
Whether this reflects a deeper connection to $G_2$ curvature is unknown.
\end{remark}
...
\bibitem{GanSavin2023}
W.~T.~Gan and G.~Savin,
\emph{The local Langlands conjecture for $G_2$},
Ann.\ Math.\ \textbf{197} (2023), 1--57.
...
\bibitem{Turcas2018}
G.~Turcas,
\emph{On Fermat's equation over some quadratic imaginary number fields},
Res.\ Number Theory \textbf{4} (2018), article~24.

\bibitem{FLHS2015}
N.~Freitas, B.~V.~Le~Hung, and S.~Siksek,
\emph{Elliptic curves over real quadratic fields are modular},
Invent.\ Math.\ \textbf{201} (2015), 159--206.
