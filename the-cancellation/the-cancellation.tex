...
\begin{enumerate}[label=(\roman*)]
  ...
  \item For $n \equiv 0 \pmod{3}$ and even: $\alpha^n + \beta^n = \pm 2^{n+1}$.
    ...
    construction (Theorem~\ref{thm:sextic}) does.
  \item For $n \equiv 0 \pmod{4}$, $n \geq 8$:
    $\alpha^n + \beta^n = -2^n$, but $c^n \geq 0$ for purely imaginary $c$.
    Open for general elements.
\end{enumerate}
\end{proposition}
...
\begin{proof}
The genus formula for compact Shimura curves $X_B(N)$ (no cusps) is
\[
  2g - 2 = \frac{\mu}{6} - \frac{\nu_2}{4} - \frac{\nu_3}{3},
\]
where $\mu = N \prod_{p \mid \mathrm{disc}(B)} (p-1)$, and
$\nu_2$, $\nu_3$ count elliptic points.
For $B$ of discriminant~6 at level $N=1$:
$\mu = 1 \cdot (2-1)(3-1) = 2$, $\nu_2 = \nu_3 = 0$.
So $2g - 2 = 2/6 < 0$, giving $g = 0$.
\end{proof}
