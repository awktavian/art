...
For general odd~$p \geq 5$, FLT over $\Z[i]$ reduces to
FLT over $\Q(i)$.
Any solution with all-real entries is a solution in $\Z$,
contradicting Wiles.
For solutions with nonzero imaginary parts, one needs that
every semistable elliptic curve over $\Q(i)$ is modular;
this follows from recent modularity results over CM fields
(see Thorne~\cite{Thorne2021} and subsequent work),
extending the Wiles/BCDT method via automorphic base change.
We note that $\Q(i)$ is not totally real, so the
Freitas--Le~Hung--Siksek theorem~\cite{FLHS2015} does not
apply directly, and a complete self-contained proof
of FLT over $\Q(i)$ for all $p$ has not appeared as a
single reference in the literature.
...
\bibitem{Thorne2021}
J.~Thorne,
\emph{Elliptic curves over $\Q_\infty$ and other totally real fields},
preprint, arXiv:2106.07287 (2021).
