...
and integer octonions $\OO_\Z$ for every odd integer $p \geq 3$.
...
(29 files, 355 declarations, zero \texttt{sorry}, zero axioms;
repository and build instructions in Section~\ref{sec:lean}).
\end{abstract}
...
Li--Yuan~\cite{LiYuan2025} on Fermat and Catalan equations over $2 \times 2$ integer matrices,
P.~Pollack~\cite{PPollack2018} on Waring's problem for integral quaternions,
A.~Pollack~\cite{APollack2020} on modular forms for $G_2$,
...
The formalization comprises 29 files, 9{,}102 lines, and 355 Lean declarations
(theorems, lemmas, and supporting definitions)
with zero \texttt{sorry} and zero axioms beyond Lean's foundations.
...
Theorem~\ref{thm:zerodiv} requires an associative ring, and the sedenions
are not associative.
Instead, we use a \emph{periodicity argument}: $\alpha^2 = \beta^2 = -1$
(verified by \texttt{native\_decide}), so $\alpha^4 = \beta^4 = 1$.
Setting $\gamma = \alpha + \beta$, one also verifies $\gamma^4 = 1$.
All three elements have multiplicative order dividing~4, so it suffices to
check the Fermat equation $\alpha^n + \beta^n = \gamma^n$ for
$n \in \{1, 2, 3, 4\}$, each verified by \texttt{native\_decide}.
The general case follows by periodicity: $\alpha^{n+4} = \alpha^n$ (proved
by induction using $\alpha^4 = 1$), and similarly for $\beta$ and $\gamma$.

This avoids any appeal to associativity.
The full proof is in \texttt{SedenionZeroDivisor.lean},
theorem \texttt{sedenion\_fermat\_all\_exponents}.
\end{example}

\begin{remark}
Alternatively, one can observe that the generated subalgebra
$\langle \alpha, \beta \rangle$ inside $\Se$ is at most 4-dimensional
and contained in an associative subalgebra (since $\alpha\beta = \beta\alpha = 0$
forces all mixed products to vanish). In this subalgebra,
Theorem~\ref{thm:zerodiv} applies directly.
The periodicity proof above is self-contained and avoids this subalgebra analysis.
\end{remark}

\begin{remark}
This is a \emph{phase transition} in the Cayley-Dickson tower.
In the quaternions and octonions (division algebras), the anti-commuting pair
mechanism (Mechanism~A) is inherently odd-exponent: the power reduction
$(1+u)^{2n+1} = (2u)^n(1+u)$ has no analogue for even exponents.
The question of whether nontrivial even-exponent solutions exist in
$\HH_\Z$ or $\OO_\Z$ remains open.
In the sedenions, zero divisors bypass the even/odd distinction entirely:
the periodicity proof gives $\alpha^n + \beta^n = \gamma^n$ for \emph{all} $n \geq 1$.

More precisely, the Cayley-Dickson algebra at dimension $2^k$ has $\binom{2^k - 1}{2}$
...
\begin{tabular}{@{}lcccc@{}}
  ...
  $\R$ & 1 & 0 & No (Wiles) & No \\
  $\C$ & 2 & 0 & No (Wiles) & No \\
  $\HH$ & 4 & $\binom{3}{2} = 3$ & Open & No \\
  $\OO$ & 8 & $\binom{7}{2} = 21$ & Open & No \\
