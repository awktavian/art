...
We discuss its potential application beyond division algebras
and identify conjugate-pair cancellation as a third route to cube-sum identities.
...
Mechanism~A is inherently odd-exponent; whether nontrivial even-exponent solutions
exist in $\HH_\Z$ or $\OO_\Z$ remains open.
\end{remark}
...
\begin{tabular}{@{}lcccc@{}}
  ...
  $\Se$ & 16 & $\binom{15}{2} = 105$ & Open$^\dagger$ & Open$^\dagger$ \\
  \bottomrule
\end{tabular}

\smallskip\noindent
{\footnotesize ${}^\dagger$Theorem~\ref{thm:zerodiv} requires associativity;
the sedenions are non-associative and the standard zero-divisor pairs have
$\alpha^2 = -2 \neq -1$, obstructing both the binomial collapse and periodicity
arguments (see Remark~7.3).}
\end{table}
